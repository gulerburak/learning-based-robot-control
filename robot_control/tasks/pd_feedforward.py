"""PD control with an inverse-dynamics feedforward term.

The feedforward term uses the desired state only, so it cannot correct an error. Without
the feedback term the robot leaves the path, because a small error grows.

    python -m robot_control.tasks.pd_feedforward
"""
...
from robot_control.controllers import ctrl_fb_pd, ctrl_ff_feedforward
...
def main():
    ...
    report_tracking_error(traj_ts, sim_ts, "PD with feedforward")
    save_tracking_plots(traj_ts, sim_ts, "pd_feedforward")
