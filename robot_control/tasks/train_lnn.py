...
Run `python -m robot_control.tasks.collect_dataset` first.

    python -m robot_control.tasks.train_lnn
    python -m robot_control.tasks.train_lnn --epochs 5     # a quick check
...
from robot_control.common import CHECKPOINT_DIR, DATASET_DIR, OUTPUT_DIR
from robot_control.lnn_training import load_datasets, run_lnn_training
from robot_control.tasks.collect_dataset import DATASET_NAME
...
def main():
    ...
    plot_convergence(
        val_loss_history, train_metrics_history, OUTPUT_DIR / "lnn_convergence.pdf"
    )
