...
def run_dataset(name: str, num_epochs: int, grid):
    ...
    plot_data(
        ...
        INPUT_COLUMNS,
        OUTPUT_COLUMNS,
        filepath=str(OUTPUT_DIR / f"gp_dynamics_dataset_{name}.pdf"),
    )
    ...
    print(f"\n=== {name} ===")
    model.print_info()
    model.plot_convergence(OUTPUT_DIR / f"gp_dynamics_convergence_{name}.pdf")
    ...
    plot_phase_portrait(
        ...
        OUTPUT_DIR / f"gp_dynamics_phase_portrait_{name}.pdf",
    )
    return model
