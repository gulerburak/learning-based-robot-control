...
many runs and removes most of the error, although the model stays wrong.
    ...
    python -m p2_control.tasks.pd_ilc
    python -m p2_control.tasks.pd_ilc --iterations 20    # a quick check
"""
...
from p2_control.ilc_pd import run_pd_ilc
...
def main():
    ...
    save_tracking_plots(traj_ts, sim_ts, "2d-2_pd_ilc")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    plot_configuration_space_ilc_convergence(
        traj_ts, ilc_its, filepath=str(OUTPUT_DIR / "2d-2_pd_ilc_convergence.pdf")
    )
